        ...
        if (pixhit) {
          if (pixhit->isValid()) {
            ...
            if ((subid == PixelSubdetector::PixelBarrel) || (subid == PixelSubdetector::PixelEndcap)) {
              ...
              /*
                            GlobalPoint global_pos = geomDet->surface().toGlobal(pixhit->localPosition());
                            printf("x,y,z = %.2f %.2f %.2f \n", global_pos.x(), global_pos.y(), global_pos.z());
                            auto trackLocalTraj  = tTrack.stateOnSurface(global_pos);
                            auto localParam  = trackLocalTraj.localParameters();
                            trkcota = localParam.dxdz();
                            trkcotb = localParam.dydz();
...
void Phase2PixelNtuple::fillPRecHit(const int detid_db,
  ...
  recHit_.xloc = pixHit->localPosition().x();
  recHit_.yloc = pixHit->localPosition().y();
  recHit_.xxloc = pixHit->localPositionError().xx();
  recHit_.xyloc = pixHit->localPositionError().xy();
  recHit_.yyloc = pixHit->localPositionError().yy();
